...
//echo $sql1;
    ...
    echo '
                     <tr>
                        <td align=left colspan=5>' . $company . '<br>' . $address . '<br>' . $town . ' - ' . $postal . '
                            <br>' . $tel . '</td>
                        <td colspan=3><center><b>COLLECTION DETAIL REPORTS</b></center></td>
	...
	$sql="SELECT if(r.rev_desc<>'',r.rev_desc,r.prec_desc) as Description,r.pay_mode, 
        sum(IF(r.pay_mode='CAS' and mpesaref='', r.total,'')) AS Cash,
		sum(IF(r.pay_mode='MPESA' OR mpesaref<>'' , r.total,'')) AS Mpesa FROM care_ke_receipts r 
...
function getReceiptsSummary($cashpoint, $shiftNo, $cashier) {
    ...
    $sql = "SELECT b.ref_no,b.patient,b.name,b.payer,b.pay_mode,SUM(b.total) AS total,
    ...
    echo '
                     <tr>
                        <td align=left>' . $company . '<br>' . $address . '<br>' . $town . ' - ' . $postal . '
                            <br>' . $tel . '</td>
                        <td colspan=5 align="center"><b>COLLECTION SUMMARY BY PATIENT REPORT</b></td>
