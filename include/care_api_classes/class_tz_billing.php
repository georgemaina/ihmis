...
//        $bill_number = $this->checkBillEncounter($encounter_no);
        ...
        while ($row = $this->result->FetchRow()) {
            ...
            $desc=addslashes($row['article']);
                $this->sql = "INSERT INTO care_ke_billing (pid, encounter_nr,bill_date,`ip-op`,bill_number,
                            ...
                            values('$row[pid]','$row[encounter_nr]','$billDate','$row[encounter_class_nr]','$row[bill_number]'
                            ,'$row[drug_class]','$row[price]','$desc','$row[notes]','$row[prescribe_date]','$row[dosage]'
                            ,'$row[times_per_day]','$row[days]','$row[prescriber]','$row[article_item_number]','$row[partcode]'
                            ,'pending','$row[qty]','$total','$row[partcode]',0
    ...
    //                 values('$insuranceid','$insuranceid','" . date("Y-m-d") . "','" . $row['encounter_class_nr']
        ...
        if ($IS_PATIENT_INSURED) {
            ...
            //            }
                ...
                $sql3="Update care_encounter_prescription set
                    bill_status='pending', bill_number='".$new_bill_number."' where status='pending' and 
