/*
 * File: app/view/Portal.js
 *
 * This file was generated by Sencha Architect version 4.3.2.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 7.6.x Classic library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 7.6.x Classic. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('CarePortal.view.Portal', {
    extend: 'Ext.container.Viewport',
    alias: 'widget.portal',

    requires: [
        'CarePortal.view.PortalViewModel',
        'CarePortal.view.Home',
        'Ext.panel.Panel',
        'Ext.form.field.Display',
        'Ext.toolbar.Spacer',
        'Ext.button.Button',
        'Ext.Img'
    ],

    viewModel: {
        type: 'portal'
    },
    defaultListenerScope: true,

    layout: {
        type: 'border',
        padding: '0 5 5 5'
    },
    items: [
        {
            xtype: 'panel',
            region: 'west',
            split: true,
            id: 'app-options',
            itemId: 'menusPanel',
            maxWidth: 400,
            minWidth: 250,
            width: 250,
            collapsible: true,
            title: 'Modules Select',
            layout: {
                type: 'accordion',
                animate: true
            }
        },
        {
            xtype: 'panel',
            region: 'center',
            itemId: 'detailsPanel',
            scrollable: true,
            layout: {
                type: 'vbox',
                align: 'stretch'
            },
            items: [
                {
                    xtype: 'home'
                }
            ]
        },
        {
            xtype: 'container',
            region: 'north',
            height: 39,
            margin: '0 0 2 0',
            layout: 'border',
            items: [
                {
                    xtype: 'container',
                    region: 'east',
                    dock: 'left',
                    layout: {
                        type: 'hbox',
                        align: 'stretch'
                    },
                    items: [
                        {
                            xtype: 'displayfield',
                            flex: 1,
                            width: 165,
                            fieldLabel: 'Welcome',
                            labelStyle: 'font-size: large;font-weight: bold;color: green;',
                            value: 'Admin',
                            fieldStyle: 'font-size: large;font-weight: bold;color: green;',
                            listeners: {
                                render: 'onDisplayfieldRender'
                            }
                        },
                        {
                            xtype: 'tbspacer',
                            flex: 1,
                            width: 47
                        },
                        {
                            xtype: 'button',
                            height: 42,
                            width: 110,
                            text: 'Intranet Email'
                        },
                        {
                            xtype: 'button',
                            height: 44,
                            itemId: 'systemAdmin',
                            width: 119,
                            text: 'System Admin'
                        },
                        {
                            xtype: 'button',
                            height: 39,
                            width: 125,
                            text: 'Special Tools'
                        },
                        {
                            xtype: 'button',
                            height: 47,
                            itemId: 'logout',
                            width: 86,
                            text: 'logout'
                        }
                    ]
                },
                {
                    xtype: 'container',
                    region: 'west',
                    dock: 'right',
                    width: 150,
                    items: [
                        {
                            xtype: 'image',
                            height: 50,
                            margin: '0 100 0 0',
                            width: 140,
                            src: 'resources/care_logo.gif',
                            listeners: {
                                afterrender: 'onImageAfterRender'
                            }
                        }
                    ]
                },
                {
                    xtype: 'container',
                    region: 'west',
                    dock: 'right',
                    width: 440,
                    items: [
                        {
                            xtype: 'displayfield',
                            itemId: 'companyTitle',
                            margin: '0 0 0 50',
                            width: 423,
                            value: 'Company Name',
                            fieldStyle: 'font-size: large;font-weight: bold;color: maroon;',
                            listeners: {
                                afterrender: 'onCompanyTitleAfterRender'
                            }
                        }
                    ]
                }
            ]
        }
    ],

    onDisplayfieldRender: function(component, eOpts) {
        component.setValue(localStorage.getItem('UserName'));
    },

    onImageAfterRender: function(component, eOpts) {
                component.setSrc(localStorage.getItem('logoPath'));

    },

    onCompanyTitleAfterRender: function(component, eOpts) {
         component.setValue(localStorage.getItem("CompanyName"));
    }

});