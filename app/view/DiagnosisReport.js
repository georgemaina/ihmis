/*
 * File: app/view/DiagnosisReport.js
 *
 * This file was generated by Sencha Architect version 4.3.2.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 7.6.x Classic library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 7.6.x Classic. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('CarePortal.view.DiagnosisReport', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.diagnosisreports',

    requires: [
        'CarePortal.view.DiagnosisReportViewModel',
        'CarePortal.view.DiagnosisReportViewController',
        'CarePortal.view.ExportButton',
        'Ext.view.Table',
        'Ext.form.FieldSet',
        'Ext.form.field.Date',
        'Ext.form.field.Number',
        'Ext.form.field.ComboBox',
        'Ext.button.Button',
        'Ext.grid.column.Column',
        'Ext.toolbar.Paging',
        'Ext.grid.plugin.Exporter'
    ],

    controller: 'diagnosisreports',
    viewModel: {
        type: 'diagnosisreports'
    },
    columnLines: true,
    store: 'DiagnosisReportStore',

    dockedItems: [
        {
            xtype: 'fieldset',
            dock: 'top',
            height: 157,
            width: 100,
            layout: 'absolute',
            items: [
                {
                    xtype: 'datefield',
                    x: -1,
                    y: 5,
                    itemId: 'startDate',
                    fieldLabel: 'From Date',
                    labelWidth: 80
                },
                {
                    xtype: 'datefield',
                    x: -1,
                    y: 40,
                    itemId: 'endDate',
                    fieldLabel: 'To Date',
                    labelWidth: 80
                },
                {
                    xtype: 'textfield',
                    x: 0,
                    y: 75,
                    itemId: 'pid',
                    fieldLabel: 'PID',
                    labelWidth: 80
                },
                {
                    xtype: 'numberfield',
                    x: 290,
                    y: 5,
                    itemId: 'age1',
                    fieldLabel: 'Between Age',
                    labelWidth: 80
                },
                {
                    xtype: 'combobox',
                    x: 290,
                    y: 75,
                    itemId: 'status',
                    fieldLabel: 'Dead/Alive',
                    labelWidth: 80,
                    store: [
                        'Dead',
                        'Alive'
                    ]
                },
                {
                    xtype: 'numberfield',
                    x: 290,
                    y: 40,
                    itemId: 'age2',
                    fieldLabel: 'And Age',
                    labelWidth: 80
                },
                {
                    xtype: 'combobox',
                    x: 570,
                    y: 75,
                    itemId: 'icd10',
                    width: 380,
                    fieldLabel: 'ICD 10 Code',
                    labelWidth: 80,
                    displayField: 'Description',
                    minChars: 2,
                    queryMode: 'local',
                    store: 'DiseasesStore',
                    typeAhead: true,
                    valueField: 'IcdCode'
                },
                {
                    xtype: 'combobox',
                    x: 570,
                    y: 40,
                    itemId: 'visits',
                    fieldLabel: 'Visit',
                    labelWidth: 80,
                    store: [
                        'New',
                        'Revisit'
                    ]
                },
                {
                    xtype: 'combobox',
                    x: 570,
                    y: 5,
                    itemId: 'gender',
                    fieldLabel: 'Gender',
                    labelWidth: 80,
                    store: [
                        'Male',
                        'Female'
                    ]
                },
                {
                    xtype: 'button',
                    x: 85,
                    y: 110,
                    itemId: 'cmdPreviewDiagnosis',
                    width: 170,
                    iconCls: 'x-fa fa-search',
                    text: '<b>Preview</b>'
                },
                {
                    xtype: 'button',
                    x: 375,
                    y: 110,
                    itemId: 'cmdPrintDiagnosis',
                    width: 170,
                    iconCls: 'x-fa fa-print',
                    text: '<b>Print</b>'
                },
                {
                    xtype: 'exportbutton',
                    itemId: 'ExportReports',
                    width: 170,
                    x: 655,
                    y: 110
                }
            ]
        },
        {
            xtype: 'pagingtoolbar',
            dock: 'bottom',
            width: 360,
            displayInfo: true,
            store: 'DiagnosisReportStore'
        }
    ],
    columns: [
        {
            xtype: 'gridcolumn',
            width: 79,
            dataIndex: 'PID',
            text: 'Pid'
        },
        {
            xtype: 'gridcolumn',
            width: 214,
            dataIndex: 'Names',
            text: 'Names'
        },
        {
            xtype: 'gridcolumn',
            width: 153,
            dataIndex: 'Date',
            text: 'Date'
        },
        {
            xtype: 'gridcolumn',
            dataIndex: 'Gender',
            text: 'Gender'
        },
        {
            xtype: 'gridcolumn',
            dataIndex: 'Age',
            text: 'Age'
        },
        {
            xtype: 'gridcolumn',
            dataIndex: 'ADMNO',
            text: 'Admno'
        },
        {
            xtype: 'gridcolumn',
            dataIndex: 'PatientStatus',
            text: 'Status'
        },
        {
            xtype: 'gridcolumn',
            dataIndex: 'DiagnosisCode',
            text: 'Diagnosis Code'
        },
        {
            xtype: 'gridcolumn',
            width: 236,
            dataIndex: 'Description',
            text: 'Description'
        },
        {
            xtype: 'gridcolumn',
            dataIndex: 'Visit',
            text: 'Visit'
        }
    ],
    plugins: [
        {
            ptype: 'gridexporter'
        }
    ],
    listeners: {
        documentsave: 'onGridpanelDocumentsave',
        beforedocumentsave: 'onGridpanelBeforeDocumentsave',
        dataready: 'onGridpanelDataReady'
    }

});