...
switch ($task) {
    ...
    case "getDentalServices":
        ...
        break;
    case "updateOpMobidity":
        updateOpMobidity($startDate,$endDate);
        break;
    case "getEncounterFromBill":
        getEncounterFromBill($billNumber);
        break;
    default:
        echo "{failure:true}";
        break;
...
function getEncounterFromBill($billNumber){
    global $conn;
    $debug=false;

    $sql="SELECT encounter_nr,encounter_class_nr,encounter_date,discharge_date FROM care_encounter where bill_number='$billNumber'";
    ...
    if($request->num_rows>0){
        while($row[]=$request->fetch_assoc()){
         ...
         echo "{'Error':'Unable to get Encounter No'}";
     }
...
function updateOpMobidity($startDate,$endDate){
    global $db;
    $debug=false;

    $sMonth=new DateTime($startDate);
    $eMonth=$sMonth->format('F');
    $startDay=$sMonth->format('dd');

    $eMonth=new DateTime($endDate);
    $endDate=$sMonth->format('F');
    $endDay=$sMonth->format('dd');
...
function getOPInvoices($startDate,$endDate,$pid){
    ...
    while ($row = $results->FetchRow()) {
        ...
        $diag=getPatientDiagnosis($row['encounter_nr']);
        $diagnosis=preg_replace('/[^a-zA-Z0-9_ -]/s', '', $diag);
